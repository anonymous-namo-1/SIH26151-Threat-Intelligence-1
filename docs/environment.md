...
| `ARGUS_GATEWAY_URL` | Next.js/Vercel build | External HTTPS gateway origin used for same-origin `/api/*` rewrites; pinned by the repository Vercel build commands |
...
For the hybrid Vercel topology, only `CLERK_PUBLISHABLE_KEY` and
`CLERK_PROXY_URL` need Vercel environment settings. The repository build command
supplies `ARGUS_GATEWAY_URL`. Keep all backend
